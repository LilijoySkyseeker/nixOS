# torrent — recovery runbook

Covers recovery once torrent has landed the FDE/Secure Boot/TPM2 +
impermanence plan in `TODO.md`. `hosts/torrent/disko.nix` already has
...
Unlike homelab, torrent has **no independent offsite backup of its
own** — its only backup destination is homelab's `zbackup` pool. Note:
the dataset stubs already in `hosts/homelab/disko.nix` for this host's
backups are named `backup/legion`/`backup-bulk/legion`, not `torrent`
— confirm at recovery time whether that's a leftover/prior hostname
for this same machine or a naming mismatch that needs reconciling
before Phase 2 ships here; don't assume `zbackup/backup/legion` is
definitely torrent's data without checking. This runbook uses
`zbackup/backup/legion` per that existing dataset naming, flagged as
unverified.

- torrent's disaster recovery is only as good as homelab's zbackup
  pool being alive and reachable — if both machines fail at once (or
  homelab's zbackup pool is what died), there's no torrent-specific
  fallback.
- Phase 2 should not ship on torrent until syncoid is actually running
  and the correct backup dataset has real, current snapshots — confirm
  with `zfs list -t snapshot -r zbackup/backup/legion` (or the correct
  dataset name once reconciled) on homelab before trusting this
  runbook.

## 0. Prerequisites

- The recovery passphrase `enrollRecovery` generated for
  `zroot-crypt` at torrent's original install is escrowed somewhere
  reachable without needing to decrypt torrent itself (see homelab's
...
- sbctl PKI backup for torrent exists outside the host.
- torrent's SSH host key / sops age identity (`torrent-machine`,
  `torrent-age` in `.sops.yaml`) is backed up outside the host.
- On homelab: the correct backup dataset for torrent (see naming note
  above) has a recent snapshot.
...
Confirm sbctl's PKI directory is in torrent's `environment.persistence`
and covered by whatever backup torrent ends up with (currently: none
independent of homelab's zbackup — see the note at the top of this
file. It should probably ride along in the same backup dataset once
that's wired up, same as its state dataset).

## 3. Scenario: full hardware failure / disk death

Torrent has a single-disk `zroot` (`zroot-crypt`) with `local/root`
and `local/home` as separate ZFS datasets (`hosts/torrent/disko.nix`)
— once the impermanence retrofit lands, home directories will NOT be
wiped by the root rollback and are the most valuable uniquely-torrent
data (root/state becomes reproducible from the flake + homelab's
backup dataset for this host).
...
2. **Re-provision via disko.** As configured (no `passwordFile`/
   ...
   again.
   ```
   disko --mode disko /path/to/flake#torrent-disko
   ```
...
5. **Enroll the TPM2 slot**:
   ...
   ```
6. **Repopulate data** — torrent has no local backup source, pull from
   homelab:
   ```
   # run from homelab, or from torrent over the tailnet against homelab
   zfs send zbackup/backup/legion@<latest-state-snapshot> | zfs receive -F zroot/local/state
   zfs send zbackup/backup-bulk/legion@<latest-home-snapshot> | zfs receive -F zroot/local/home
   ```
   Confirm the actual dataset names (see the `legion`-vs-`torrent`
   naming note above) and the target names against whatever the
   impermanence retrofit lands as torrent's real dataset layout — a
...
7. **Restore sops secrets access** — same chicken-and-egg as homelab's
   Scenario 3 step 7. Preferred: restore torrent's actual backed-up
   ...
   has access (homelab or thinkpad), update `.sops.yaml`, `sops
   updatekeys`, rebuild, rotate.
8. **First `nixos-rebuild switch`** should now succeed. Confirm
   sanoid/syncoid resume (syncoid direction here is torrent → homelab
...
- Reconcile the `legion`-vs-`torrent` dataset naming in
  `hosts/homelab/disko.nix` before Phase 2 ships on this host.
- Confirm real post-impermanence dataset names once that retrofit
  lands, update Scenario 3 step 6.
- Don't ship Phase 2 on torrent until its backup dataset has real
  current snapshots (syncoid wiring is being built separately).
