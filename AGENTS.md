...
| `docs/procedures/testing-changes.md` | Which validation layer to reach for (`nixfmt`/lint → `nix flake check` → `nixos-rebuild build` → VM test → `nvd diff` → switch), maps those layers onto the `workflow` skill's trust hierarchy (`docs/skills/workflow/reference.md`), and what's automated (git hooks, plus the `workflow` skill's `verify-ladder` gate). |
