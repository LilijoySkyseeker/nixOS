...
For a fast, pre-computed answer (enabled services, packages, containers,
storage, firewall, timers, users, secrets in use), check that host's own
`hosts/<name>/README.md` "Host Inventory" block first — generated by
`scripts/doc-host.sh` from the evaluated config, but not guaranteed
current between runs. The steps below are how to derive the same answer
from source when the block is missing, stale, or doesn't cover what
you're after (e.g. ZFS pool topology, NFS/Samba per-share detail).
