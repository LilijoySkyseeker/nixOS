...
> ### DONE 2026-09-01 — unblocked by physical access to both laptops
>
> The user had physical access to torrent and thinkpad, which cleared the
> obstacle that mattered: someone at the console. Sequence actually
> followed:
>
> 1. **Both laptops deployed to this audit branch first.** torrent turned
>    out to already be nearly there (a stale `RESUME.md` claim corrected
>    in the same session — see its eighth/ninth-session notes) so this
>    step was a small `switch`, not the feared 40-commit jump; thinkpad
>    was a normal first deploy at its console.
> 2. New keypair generated by the user; private half re-encrypted into
>    `secrets/secrets.yaml`, public half (`homelab-zrepl-puller`) placed in
>    `modules/flake/vars.nix`.
> 3. **`sops.secrets.homelab_zrepl_key` given `restartUnits = [
>    "zrepl.service" ]`**, closing the same SYS-11 gap already fixed for
>    WireGuard and factorio — not previously set on this secret. Go-netssh
>    shells out to the system `ssh` binary per connection, which plausibly
>    already re-reads `-i`'s target fresh each time (unlike WireGuard's
>    `RemainAfterExit` oneshot), but that was never verified either way, and
>    a killed pull just retries — so `restartUnits` was added rather than
>    relying on the per-invocation read being true.
> 4. Deployed **laptops first, homelab last**, per the fail-safe order
>    above. Manual `nixos-rebuild switch`, not the disabled `boot`-mode
>    auto-deploy — so the "needs a reboot" caveat above did not apply; the
>    key took effect immediately on switch.
> 5. **Verified by exercising the credential**, not by reading the
>    activation log: homelab's activation did show `modifying secret:
>    homelab_zrepl_key` and `restarting the following units: …
>    zrepl.service` (the `restartUnits` firing), but per this runbook's own
>    repeated lesson that proves nothing by itself. `zrepl signal wakeup
>    torrent` / `wakeup thinkpad` were issued and `zbackup`'s replicas were
>    watched directly: both `zbackup/backup/torrent/…` and
>    `zbackup/backup/thinkpad/…` picked up a fresh snapshot
>    (`@zrepl_20260901_175824_000` / `@zrepl_20260901_175816_000`) matching
