...
- **Corrected 2026-09-15 — the 2026-09-11 figure above is wrong by 21x, and the original instinct was right.** That number came from a 90-second sample taken minutes after the deploy, so it measured deploy churn, not logging. Over **3.87 days of actual steady state** the dataset grew to 108 MiB: **27.9 MiB/day** for homelab, i.e. **~0.8 GiB per 30-day retention window for homelab alone and ~3.3 GiB if all four hosts ship at a similar rate** — against 153 GiB free. So the volume concern is negligible at normal rates: the 50 GB quota floated first would have been ample (~15x headroom), the ~100 GiB "correction" was over-cautious, and D3's 30-day retention is comfortably predictable after all. A quota is still worth setting, but purely as a runaway guard against F9's abuse case (~345 GB/day at the default distributor limit), not as capacity planning. **Lesson: never size anything from a sample taken during or just after a deploy.**
- **Measured 2026-09-11, and it moves the recommended number a lot.** Real ingest on the live host, sampled over 90s with only homelab shipping: **7.1 KiB/s ≈ 596 MiB/day**, i.e. **~17.5 GiB at 30-day steady state for homelab alone**, and **~70 GiB if all four hosts ship at a similar rate** (homelab is the busiest host, so that is an upper-ish bound, and the sample includes this deploy's own churn). Against 153 GiB free on `zroot` that is a real fraction of the pool, and it means the **50 GB quota suggested in passing earlier would have been too small — it would have throttled normal fleet logging, not a runaway.** A quota is still the right guard, but it belongs near ~100 GiB (above projected steady state, well below filling the pool), and the number should be revisited once all four hosts actually ship (blocked on G11). Also worth re-reading D3 with this figure in hand: 30-day retention was chosen so "disk use stays predictable", and ~70 GiB may be more than was pictured — shorter retention or D8's per-host filtering are the levers if so.
